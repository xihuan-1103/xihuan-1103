/**
 * 分包清单
 * 与收入合同清单（ContractInventoryList）相同的设计与交互：
 *  - 列表页：分包合同台账（清单状态流转：草稿 → 待确认 → 已确认）
 *  - 「创建分包清单」：选择分包合同后进入维护
 *  - 维护页：复用 ContractInventoryMaintenance（按年份/专业分组的清单树、
 *    细目增改、修改记录、样表模拟导入），存储前缀为 SUB_CONTRACT_INVENTORIES_
 */

import React, { useState } from 'react';
import {
  Search, Plus, RotateCcw, Eye, FileText, Check, X, AlertCircle, CheckCircle2,
  Briefcase,
} from 'lucide-react';
import { cn } from '@/lib/utils';
import type { SubContract } from './types';
import {
  getSubContracts, getSubInventories, seedSubInventories,
  getSubInventoryStatus, setSubInventoryStatus, SUB_INVENTORY_PREFIX, SUB_SAMPLE_FILES,
} from './subcontractStore';
import ContractInventoryMaintenance from '@/components/ContractInventoryMaintenance';

interface Props { key?: string | number; }

export default function SubContractInventory({}: Props) {
  const [contracts] = useState<SubContract[]>(() => getSubContracts().filter(c => c.status === '已确认'));
  const [searchForm, setSearchForm] = useState({ name: '', code: '', partyB: '', status: '' });

  // 清单状态（草稿/待确认/已确认），按分包合同存储
  const [inventoryStatuses, setInventoryStatuses] = useState<Record<string, '草稿' | '待确认' | '已确认'>>(
    () => Object.fromEntries(contracts.map(c => [c.id, getSubInventoryStatus(c.id)])));

  // 当前进入维护的分包合同
  const [activeContract, setActiveContract] = useState<SubContract | null>(null);

  // 创建弹窗
  const [isCreateModalOpen, setIsCreateModalOpen] = useState(false);
  const [selectedIdForCreate, setSelectedIdForCreate] = useState('');
  const [createSearchQuery, setCreateSearchQuery] = useState('');

  // 快捷确认弹窗
  const [confirmingContract, setConfirmingContract] = useState<SubContract | null>(null);

  const reloadStatuses = () =>
    setInventoryStatuses(Object.fromEntries(contracts.map(c => [c.id, getSubInventoryStatus(c.id)])));

  const handleUpdateStatus = (subId: string, newStatus: '草稿' | '待确认' | '已确认') => {
    setSubInventoryStatus(subId, newStatus);
    setInventoryStatuses(prev => ({ ...prev, [subId]: newStatus }));
  };

  // 清单合价（读取分包清单结构）
  const getSubInventoryTotal = (c: SubContract) =>
    getSubInventories(c).reduce((sum, inv) => sum + (Number(inv.totalAmount) || 0), 0);

  const hasInventoriesCreated = (c: SubContract) => getSubInventories(c).length > 0;
  ...
  const filteredContracts = contracts.filter(c => {
    ...
    return matchName && matchCode && matchParty && matchStatus;
  });

  const handleOpenCreateModal = () => {
    setIsCreateModalOpen(true);
    setSelectedIdForCreate('');
    setCreateSearchQuery('');
  };

  const handleConfirmCreate = () => {
    if (!selectedIdForCreate) { alert('请选择一个分包合同！'); return; }
    const contract = contracts.find(c => c.id === selectedIdForCreate);
    if (contract) {
  ...
  const getStatusBadgeStyle = (status: '草稿' | '待确认' | '已确认') => {
    switch (status) {
      case '已确认': return 'bg-emerald-50 border-emerald-200 text-emerald-700';
      case '待确认': return 'bg-amber-50 border-amber-200 text-amber-700';
    ...
    return (
      <div className="bg-slate-50 min-h-screen">
        <ContractInventoryMaintenance
          ...
          onBack={() => { setActiveContract(null); reloadStatuses(); }}
          storagePrefix={SUB_INVENTORY_PREFIX}
          heading="分包清单维护"
          seedInventories={seedSubInventories(activeContract)}
          sampleFiles={SUB_SAMPLE_FILES}
        />
      </div>
    );
  }

  const availableContractsForCreate = contracts.filter(c => {
    ...
    <div className="space-y-4 font-sans text-slate-800">
      ...
      <div className="bg-white p-4 rounded-lg shadow-2xs border border-gray-200">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-x-4 gap-y-3 items-center">
          ...
          <div className="flex items-center gap-2">
            <label className="text-xs font-medium text-slate-600 shrink-0 w-18 text-right">清单状态:</label>
        ...
        <div className="flex justify-end items-center gap-2.5 mt-3 pt-3 border-t border-gray-100">
          ...
          <button onClick={() => setSearchForm({ name: '', code: '', partyB: '', status: '' })}
      ...
      <div className="bg-white rounded-lg shadow-2xs border border-gray-200 overflow-hidden">
        <div className="p-4 border-b border-gray-200 bg-slate-50/50 flex justify-between items-center">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-sm text-slate-800">分包清单台账列表</h3>
          ...
          <table className="w-full text-xs text-left border-collapse">
            <thead className="bg-[#FAFBFD] text-slate-500 font-bold border-b border-gray-200">
              <tr>
                ...
                <th className="px-4 py-3 text-right font-mono">分包合同总金/元</th>
            ...
            <tbody className="divide-y divide-gray-150">
              {filteredContracts.length === 0 ? (
                ...
                filteredContracts.map((contract, index) => {
                  ...
                  const totalInventoryAmount = getSubInventoryTotal(contract);
                  const isExceeded = totalInventoryAmount > contract.amount;
                  const isCreated = hasInventoriesCreated(contract);

                  return (
                    <tr key={contract.id} className="hover:bg-blue-50/30 transition-colors group">
                      <td className="px-3.5 py-3 text-center text-slate-400 font-mono font-medium">{index + 1}</td>
                      <td className="px-4 py-3 font-bold text-slate-800">
                        <button onClick={() => setActiveContract(contract)}
                          className="group-hover:text-[#165DFF] text-left transition-colors cursor-pointer hover:underline">
                          {contract.name}
                        </button>
                      </td>
                      <td className="px-3.5 py-3 text-slate-600 font-medium">{contract.partyB}</td>
                      <td className="px-3.5 py-3 font-mono text-[11px] text-slate-600 font-medium">{contract.code}</td>
                      <td className="px-3.5 py-3 text-center font-mono text-[11px] text-slate-500">
                        <div>{contract.performanceStartDate || '-'} ~ {contract.performanceEndDate || '-'}</div>
                      </td>
                      <td className="px-3.5 py-3 text-center select-none">
                        <span className={cn('inline-flex px-2 py-0.5 rounded text-[11px] font-semibold border', getStatusBadgeStyle(status))}>
                          {status}
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-3 text-right font-mono font-bold">
                        {isCreated ? (
                          <div className="space-y-0.5">
                            <span className={cn('text-xs font-bold', isExceeded ? 'text-rose-600' : 'text-[#165DFF]')}>
                              ¥{totalInventoryAmount.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                            </span>
                            {isExceeded && (
                              <div className="text-[10px] text-rose-500 flex items-center justify-end gap-0.5" title="清单合价超过了分包合同总金额！">
                      ...
                      <td className="px-3.5 py-3 text-center font-mono text-[11px] text-slate-400">
                        {contract.confirmTime || '-'}
                      </td>
                      <td className="px-4 py-3 text-center">
                        <div className="flex items-center justify-center gap-2">
                          <button onClick={() => setActiveContract(contract)}
                            className="text-[#165DFF] hover:underline flex items-center gap-0.5 font-bold cursor-pointer text-xs" title="查看清单明细">
                            <Eye size={13} /> 查看
                          </button>
                          <button onClick={() => setActiveContract(contract)}
                            className="text-[#165DFF] hover:underline flex items-center gap-0.5 font-bold cursor-pointer text-xs" title="编辑清单内容">
                            <FileText size={13} /> 编辑
                          </button>
                          {status === '待确认' ? (
                            <button onClick={() => setConfirmingContract(contract)}
                              ...
                              title="对清单进行审核确认">
                              <Check size={11} className="stroke-[3]" /> 确认
                            </button>
                          ) : status === '草稿' ? (
                            <button onClick={() => handleUpdateStatus(contract.id, '待确认')}
        ...
        <div className="p-3.5 bg-slate-50 border-t border-gray-200 text-xs text-slate-500 flex flex-col sm:flex-row justify-between items-center gap-2 select-none">
          <div className="flex items-center gap-3">
            ...
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-slate-400" /> 草稿 (编辑维护)</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-500" /> 待确认 (送审校验)</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-emerald-500" /> 已确认 (归档生效)</span>
          </div>
          <p className="font-medium">提示：点击合同名称或操作中的编辑，可进入该合同下多份分包清单细目的导入和增改。</p>
        </div>
      </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-2xl max-w-lg w-full flex flex-col max-h-[85vh]">
            ...
            <div className="p-5 flex-1 overflow-y-auto space-y-4">
              <p className="text-[11px] text-slate-500 leading-relaxed">
                所有分包清单必须挂接在<b>已确认的分包合同</b>之下。请从下方列表中选择一个分包合同以开始创建其清单：
              </p>
              ...
              <div className="relative">
                ...
                <input type="text" placeholder="搜索分包合同名称、编码..."
                  value={createSearchQuery} onChange={e => setCreateSearchQuery(e.target.value)}
              ...
              <div className="border border-slate-150 rounded-xl overflow-hidden max-h-56 overflow-y-auto divide-y divide-slate-100">
                {availableContractsForCreate.length === 0 ? (
                  <div className="p-8 text-center text-slate-400 text-xs">没有找到可选的分包合同</div>
                ) : (
                  availableContractsForCreate.map(c => {
                    const isAlreadyCreated = hasInventoriesCreated(c);
                    const isSelected = selectedIdForCreate === c.id;
                    return (
                      <div key={c.id} onClick={() => setSelectedIdForCreate(c.id)}
                        className={cn(
                        ...
                        <div className="flex-1 pr-4">
                          <div className="flex items-center gap-1.5 font-bold text-slate-700">
                            <span className="font-mono text-[10px] bg-slate-100 text-slate-600 px-1 py-0.5 rounded font-medium">{c.code}</span>
                            <span className={cn(isAlreadyCreated && 'line-through text-slate-400')}>{c.name}</span>
                          </div>
                          <div className="text-[10px] text-slate-400 mt-1">
                            分包商: {c.partyB} | 金额: ¥{c.amount.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                          </div>
                        </div>
                        <div className="shrink-0 select-none">
                          {isAlreadyCreated ? (
                            <span className="text-[10px] font-bold text-slate-400 bg-slate-100 border border-slate-250 px-2 py-0.5 rounded-full">已创建清单</span>
                          ) : isSelected ? (
                            <span className="text-[10px] font-black text-blue-650 bg-blue-100 border border-blue-200 px-2 py-0.5 rounded-full">已选定</span>
                          ) : (
                            <span className="text-[10px] font-bold text-slate-400 border border-slate-200 px-2 py-0.5 rounded-full hover:border-blue-300 hover:text-blue-600">点击选择</span>
                          )}
                        </div>
                      </div>
                    );
              ...
              {selectedIdForCreate && hasInventoriesCreated(contracts.find(c => c.id === selectedIdForCreate)!) && (
                <div className="bg-amber-50 border border-amber-200 p-3 rounded-xl flex items-start gap-2 text-[11px] text-amber-700 leading-relaxed font-medium">
                  ...
                  <div>
                    该分包合同已经创建过清单了。您应直接在列表页点击该合同的<b>“查看”</b>或<b>“编辑”</b>来维护、增改其现有的多份分包清单。
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="p-4 border-t border-slate-100 bg-slate-50 rounded-b-2xl flex items-center justify-end gap-2">
              <button type="button" onClick={() => setIsCreateModalOpen(false)}
                className="px-4 py-2 bg-white border border-slate-200 hover:bg-slate-100 text-slate-650 font-bold rounded-lg transition-all cursor-pointer text-xs">取消</button>
              <button type="button"
                disabled={!selectedIdForCreate || hasInventoriesCreated(contracts.find(c => c.id === selectedIdForCreate)!)}
                onClick={handleConfirmCreate}
                className={cn(
                  'px-5 py-2 text-white font-extrabold rounded-lg transition-all text-xs shadow-xs',
                  (!selectedIdForCreate || (selectedIdForCreate && hasInventoriesCreated(contracts.find(c => c.id === selectedIdForCreate)!)))
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-slate-900/60 backdrop-blur-xs p-4 select-none animate-in fade-in duration-150">
          <div className="bg-white rounded-2xl border border-slate-100 shadow-2xl max-w-md w-full flex flex-col">
            <div className="p-4 border-b border-slate-150 bg-slate-50 rounded-t-2xl flex justify-between items-center">
              ...
              <button type="button" onClick={() => setConfirmingContract(null)}
                className="p-1 hover:bg-slate-200 text-slate-400 hover:text-slate-750 rounded-lg transition-colors cursor-pointer">
                <X size={16} />
              </button>
            </div>
            ...
            <div className="p-5 space-y-4 text-xs">
              <div className="bg-amber-50 border border-amber-250 p-3.5 rounded-xl flex items-start gap-2 text-amber-800 leading-relaxed font-semibold">
                ...
                <p>
                  您正在对分包合同<b>【{confirmingContract.name}】</b>的全部清单细目进行“审核确认”操作。
                </p>
              </div>
              ...
              <div className="space-y-2 bg-slate-50 border border-slate-200 p-4 rounded-xl font-medium text-slate-650">
                ...
                <div className="flex justify-between">
                  <span>分包商:</span>
                  <span className="font-bold text-slate-800">{confirmingContract.partyB}</span>
                </div>
                <div className="flex justify-between">
                  <span>分包合同额:</span>
                ...
                <div className="flex justify-between">
                  <span>清单总合价:</span>
                  <span className="font-mono font-black text-blue-600">¥{getSubInventoryTotal(confirmingContract).toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                </div>
                <div className="flex justify-between">
                  <span>已创建清单数:</span>
                  <span className="font-bold text-slate-800">{hasInventoriesCreated(confirmingContract) ? `${getSubInventories(confirmingContract).length} 份` : '暂无'}</span>
                </div>
              </div>

              <p className="text-[11px] text-slate-400 leading-relaxed">
                确认归档后，清单状态将转为<b>“已确认”</b>。该清单将被正式锁定并可作为分包结算、计量支付的实施基准，同时可用于「清单挂接」与收入合同清单建立关联。
              </p>
            </div>
